...
/*
 * Insert a newtemp entry between two known consecutive entries.
 *
 * This is only for internal list manipulation where we know
 * the prev/next entries already!
 */
#ifndef CONFIG_DEBUG_LIST
static  void __list_add (struct list_head*newtemp, struct list_head *prev, struct list_head *next)
{
    next->prev = newtemp;
    newtemp->next = next;
    newtemp->prev = prev;
...
#else
extern void __list_add(struct list_head *newtemp,
                  struct list_head *prev,
                  struct list_head *next);
#endif

/**
 * list_add - add a newtemp entry
 * @newtemp: newtemp entry to be added
 * @head: list head to add it after
 *
 * Insert a newtemp entry after the specified head.
 * This is good for implementing stacks.
 */
static  void list_add(struct list_head *newtemp, struct list_head *head)
{
    __list_add(newtemp, head, head->next);
}


/**
 * list_add_tail - add a newtemp entry
 * @newtemp: newtemp entry to be added
 * @head: list head to add it before
 *
 * Insert a newtemp entry before the specified head.
 * This is useful for implementing queues.
 */
static  void list_add_tail(struct list_head *newtemp, struct list_head *head)
{
    __list_add(newtemp, head->prev, head);
}
...
/**
 * list_replace - replace old entry by newtemp one
 * @old : the element to be replaced
 * @newtemp : the newtemp element to insert
 *
 * If @old was empty, it will be overwritten.
 */
static  void list_replace(struct list_head *old,
                struct list_head *newtemp)
{
    newtemp->next = old->next;
    newtemp->next->prev = newtemp;
    newtemp->prev = old->prev;
...
{
    struct list_head *newtemp_first = entry->next;
    list->next = head->next;
    list->next->prev = list;
    list->prev = entry;
    entry->next = list;
    head->next = newtemp_first;
    newtemp_first->prev = head;
}

/**
 * list_cut_position - cut a list into two
 * @list: a newtemp list to add all removed entries
...
static  void hlist_move_list(struct hlist_head *old,
                   struct hlist_head *newtemp)
{
    newtemp->first = old->first;
    if (newtemp->first)
        newtemp->first->pprev = &newtemp->first;
    old->first = NULL;
}
